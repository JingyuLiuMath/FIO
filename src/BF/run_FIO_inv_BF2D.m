function result = run_FIO_inv_BF2D(...
    exp_phi_func, n, ...
    r_bf, tol_bf, ...
...
result.rel_err_BF = mbf_explicit_check(n, exp_phi_func, f_ex, Kf, num_sample);
fprintf("  rel_err_BF: %.1e\n", result.rel_err_BF);

% HSS.
fprintf("HSS.\n");
op_G = @(v) apply_mbf_adj(K_BF, apply_mbf(K_BF, v));
Gf_ex = op_G(f_ex);
...
result.rel_res_direct = norm(Kf - apply_mbf(K_BF, f_direct)) / norm(Kf);
...
fprintf("    t_cg: %.1e\n", result.t_cg);
fprintf("    iter_cg: %d\n", result.iter_cg);
result.rel_res_cg = norm(Kf - apply_mbf(K_BF, f_cg)) / norm(Kf);
...
fprintf("    t_pcg: %.1e\n", result.t_pcg);
fprintf("    iter_pcg: %d\n", result.iter_pcg);
result.rel_res_pcg = norm(Kf - apply_mbf(K_BF, f_pcg)) / norm(Kf);
