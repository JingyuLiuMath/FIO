...
assert(r_ns >= 1, "Butterfly:NewtonSchulzInverse:InvalidRank", ...
    "r_ns must be positive.");
assert(tol_ns >= 0, "Butterfly:NewtonSchulzInverse:InvalidNSTolerance", ...
    "tol_ns must be nonnegative.");
assert(maxit >= 0, "Butterfly:NewtonSchulzInverse:InvalidMaxit", ...
    "maxit must be nonnegative.");
assert(num_test >= 1, "Butterfly:NewtonSchulzInverse:InvalidNumTest", ...
...
% Estimate the largest singular value using only BF and BF' applications.
q = randn(N, 1);
q = q / norm(q);
for it = 1 : num_power_iter
    z = BF.ApplyAdjoint(BF.Apply(q));
    norm_z = norm(z);
    assert(norm_z > 0, "Butterfly:NewtonSchulzInverse:SingularOperator", ...
...
% X_0 = alpha * K'.  Its BHP factors are available directly from K.
...
residual = Residual(...
    K_inv_BF, test_mat, BF_test_mat, test_norm);
...
history.construct_tolerance(1) = nan;
...
for iter = 1 : maxit
    ...
    out_blackbox = X_new.BlackBoxConstruct(...
        op_X_next, op_X_next_adjoint, ...
        r_ns, tol_construct, 0);
    t_construct = toc;
    K_inv_BF = X_new;
...
history.blackbox_operator_time = BlackBoxField(...
    history.blackbox_stats, "operator_time");
history.blackbox_basis_time = BlackBoxField(...
    history.blackbox_stats, "basis_time");
history.blackbox_factor_time = BlackBoxField(...
    history.blackbox_stats, "factor_time");
history.blackbox_middle_time = BlackBoxField(...
    history.blackbox_stats, "middle_time");
history.blackbox_other_time = BlackBoxField(...
    history.blackbox_stats, "other_time");
...
X_BF = Butterfly(BF.n_, BF.n_leaf_, 0);
