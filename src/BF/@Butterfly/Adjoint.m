...
arguments (Output)
    BF_adjoint Butterfly;
end

assert(BF.constructed_, "Butterfly:Adjoint:NotConstructed", ...
    "Call Construct or BlackBoxConstruct before Adjoint.");

BF_adjoint = Butterfly(BF.n_, BF.n_leaf_, 0);
...
for i = 1 : size(BF_adjoint.M_, 1)
    for j = 1 : size(BF_adjoint.M_, 2)
        BF_adjoint.M_{i, j} = alpha * BF_adjoint.M_{i, j};
    end
end
