function [K_inv_BF, history] = BF_Inv_Newton_Schulz(...
    n, K_BF, r_ns, tol_ns, maxit, num_test, verbose, ...
    type_bf, type_bf_inv)
% BF_Inv_Newton_Schulz constructs a black-box BF approximate inverse.
...
assert(r_ns >= 1, "FIO:BFInverse:InvalidRank", ...
    "r_ns must be positive.");
assert(tol_ns >= 0, "FIO:BFInverse:InvalidNSTolerance", ...
    "tol_ns must be nonnegative.");
assert(maxit >= 0, "FIO:BFInverse:InvalidMaxit", ...
    "maxit must be nonnegative.");
assert(num_test >= 1, "FIO:BFInverse:InvalidNumTest", ...
    "num_test must be positive.");
assert(n >= 1 && n == floor(n), "FIO:BFInverse:InvalidSize", ...
    "n must be a positive integer.");
assert(any(type_bf == ["bf", "fbf", "mybf"]), ...
    "FIO:BFInverse:UnsupportedType", ...
    "type_bf must be bf, fbf, or mybf.");
assert(type_bf_inv == "bf", ...
    "FIO:BFInverse:UnsupportedOutputType", ...
    "type_bf_inv must be bf for a 1D SMP inverse.");

if type_bf == "mybf"
    assert(isa(K_BF, "Butterfly"), ...
        "FIO:BFInverse:InvalidButterfly", ...
        "A 1D Butterfly object is required for type_bf = mybf.");
...
for it = 1 : num_power_iter
    ...
    norm_z = norm(z);
    assert(norm_z > 0, "FIO:BFInverse:SingularOperator", ...
...
if verbose == 1
    fprintf("  Newton-Schulz SMP inverse.\n");
...
if type_X == "adjoint_view"
    if history.converged
        warning("FIO:BFInverse:InitialGuessRetained", ...
    ...
    elseif maxit == 0
        warning("FIO:BFInverse:ZeroIterationStructureRetained", ...
...
function [Factor, out] = ConstructInverseBF(...
    n, op_K, op_K_adjoint, x, xi, r_bf, tol_bf)

[Factor, out] = my_construct_bf_blackbox(...
    n, op_K, op_K_adjoint, ...
    x, xi, 0, r_bf, tol_bf, "bf");
...
u = my_apply_bf_adj(X_BF, y, type_X);
y = 2 * u - my_apply_bf_adj(...
...
residual = norm(...
    my_apply_bf(X_BF, K_test_mat, type_X) ...
    - test_mat, "fro") ...
    / test_norm;
